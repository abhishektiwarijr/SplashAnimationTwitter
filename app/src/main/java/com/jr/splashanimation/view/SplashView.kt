...
import com.jr.splashanimation.BuildConfig
...
class SplashView : View {
    ...
    private fun setupAttributes(attrs: AttributeSet?) {
        ...
        for (i in 0 until numAttrs) {
            ...
            when (attr) {
                R.styleable.SplashView_icon -> setIconDrawable(a.getDrawable(i))
                R.styleable.SplashView_iconColor -> setIconColor(
                    a.getColor(
                        i,
                        DEFAULT_ICON_COLOR
                    )
                )
                R.styleable.SplashView_holeFillColor -> setHoleFillColor(
                    a.getColor(
                        i,
                        DEFAULT_HOLE_FILL_COLOR
                    )
                )
                R.styleable.SplashView_duration -> setDuration(
                    a.getInt(
                        i,
                        DEFAULT_DURATION
                    ).toLong()
                )
                R.styleable.SplashView_removeFromParentOnEnd -> setRemoveFromParentOnEnd(
                    a.getBoolean(
                        i,
     ...
     */
    private fun initialize() { // make the background transparent so that the view does not automatically draw any unwanted colors
    ...
    fun setIconDrawable(icon: Drawable?) {
        mIcon = icon
        if (mIcon != null) {
            mIconWidth = mIcon?.intrinsicWidth ?: 0
            mIconHeight = mIcon?.intrinsicHeight ?: 0
     ...
     * @param listener
     */
    fun splashAndDisappear(listener: ISplashListener?) { // create an animator from scale 1 to max
...
// for some reason this animation can run beyond 100%
        ...
        animator.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationStart(animation: Animator) { // notify the listener of animation start (if listener is set)
                listener?.onStart()
            }

            override fun onAnimationEnd(animation: Animator) { // check if we need to remove the view on animation end
                if (mRemoveFromParentOnEnd) { // get the view parent
                    val parent = parent
                    ...
                    } else if (BuildConfig.DEBUG) { // even though we had to remove the view we either don't have a parent, or the parent does not implement the method
// necessary to remove the view, therefore create a warning log (but only do this if we are in DEBUG mode)
    ...
    override fun onDraw(canvas: Canvas) { // calculate the scaled width and height
        ...
        if (mCurrentScale < 2) { // draw a bgColored rectangle right underneath the icon, make the rectangle a little bigger using the threshold value
        ...
        if (mIcon != null) { // save the current canvas state
            canvas.save()
            ...
            canvas.restore()
        } else if (BuildConfig.DEBUG) { // if the icon is not set then log a warning message if we are in debug mode, this message will be logged every time onDraw is called
            Log.w(
                TAG,
                "icon is not set when the view needs to be drawn"
            )
        }
    }
    ...
    companion object {
        ...
        // give a stroke width to the paint so that the rectangles get a little overlap
        private const val PAINT_STROKE_WIDTH = 2
    }
}